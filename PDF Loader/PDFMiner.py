...
def load_pdf_with_pdfminer(input_path, output_path):
    # 1. 处理路径（转换为绝对路径）
    ...
    try:
        # 初始化 Loader 并加载文档
        print("正在使用 PDFMiner 解析 PDF...")
        print("提示: PDFMiner 是纯 Python 实现，擅长精细的布局分析。")
        ...
        with open(final_output_path, "w", encoding="utf-8") as f:
            ...
            header = f"文件解析结果 (PDFMiner): {os.path.basename(pdf_file_path)}\n" \
                     f"保存位置: {final_output_path}\n" \
                     f"总页数: {len(docs)}\n" \
                     f"特点: 纯 Python 解析，专注于文本布局和结构分析。\n" \
                     f"{'=' * 30}\n\n"
...
if __name__ == "__main__":
    # 初始化参数解析器
    parser = argparse.ArgumentParser(description="PDFMiner 解析工具")
    ...
    # -o 参数: 输出文件路径，默认 Output/PDFMinerLoader.txt
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="Output/PDFMinerLoader.txt",
        help="输出 txt 文件的路径 (默认: Output/PDFMinerLoader.txt)"
    )

    args = parser.parse_args()

    # 运行主逻辑
    load_pdf_with_pdfminer(args.input, args.output)